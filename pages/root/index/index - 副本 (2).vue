<template>
	<view>
		...
		<!-- :style="[{height: index_height[current]+'px'}]" -->
		<swiper class="swiper-box"
			...
			<swiper-item class="swiper-item">
				<!-- <scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom"> -->
					...
					<u-loadmore id="item" :status="loadStatus[0]" bgColor="transparent"></u-loadmore>
				<!-- </scroll-view> -->
			</swiper-item>
			<swiper-item class="swiper-item">
				<!-- <scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom"> -->
					...
					<u-loadmore id="item" :status="loadStatus[1]" bgColor="transparent"></u-loadmore>
				<!-- </scroll-view> -->
			</swiper-item>
			<swiper-item class="swiper-item">
				<!-- <scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom"> -->
					...
					<u-loadmore id="item" :status="loadStatus[2]" bgColor="transparent"></u-loadmore>
				<!-- </scroll-view> -->
			</swiper-item>
		</swiper>
		</scroll-view>
...
<script>
	...
	export default {
		...
		onLoad() {
			// this.index =  [this.index_project,this.index_talent,this.index_qa];
			...
			index_data_refresh(0,3).then((value)=>{
				...
				index_data_refresh(1,4).then((value)=>{
					...
					index_data_refresh(2,4).then((value)=>{
						...
						this.index_height[this.current]=this.getHeight();
		...
		methods: {
			getHeight(){
				// let current = this.index[this.current];
				// let sum = 0;
				// for (let i=0;i<3;i++) {
				// 	let temp = current.length-i;
				// 	uni.createSelectorQuery().in(this).select('.item'+temp).boundingClientRect(data => {
				// 		sum+=data.height;
				// 	}).exec();
				// }
				// return sum;
